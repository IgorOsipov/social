  ...
  useEffect(() => {
    ...
    if (!!parsed.page)
      currentPage !== Number(parsed.page) &&
        dispatch(actions.setCurrentPage(Number(parsed.page)));
    if (!!parsed.term)
      actualFilter.term !== parsed.term &&
        dispatch(actions.setFilter({ ...actualFilter, term: parsed.term }));
    if (!!parsed.friend)
      actualFilter.friend !== (parsed.friend as any) &&
        dispatch(
          actions.setFilter({ ...actualFilter, friend: parsed.friend as any })
        );
    if (!!parsed.size)
      pageSize !== Number(parsed.size) &&
        dispatch(actions.setPageSize(Number(parsed.size)));
