...
export const requestUsers = (
  ...
  return async (dispatch) => {
    ...
    const responce = await SamAPI.getUsers(
      currentPage,
      pageSize,
      filter.term,
      filter.friend
...
const toogleUser = async (
  ...
  userId: number,
  apiMethod: (userId: number) => Promise<apiResponceType & noDataResponceType>,
...
export const unfollowUser =
  ...
  async (dispatch) => {
    await toogleUser(
      dispatch,
      userId,
      SamAPI.unfollowUser.bind(SamAPI),
      actions.unfollow
    );
  };
