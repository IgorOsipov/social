...
export const actions = {
    setCapcthaUrl: (captchaUrl: string) => ({ type: 'SN/auth/GET_CAPCTHA_URL', captchaUrl } as const),
    setAuthUserData: (userId: number | null, email: string | null, login: string | null, isAuth: boolean) => (
        { type: 'SN/auth/SET_USER_DATA', data: { userId, email, login, isAuth } } as const)
}

export const getAuthUserData = (): ThunkType => async (dispatch) => {
    const responce = await SamAPI.authorization();
    if (responce.resultCode === responceApiCodes.Success) {
        dispatch(actions.setAuthUserData(responce.data.id, responce.data.email, responce.data.login, true));
    }
}
